...
def apply_rome_to_model(
    ...
    if return_orig_weights:
        ...
        weights_copy = save_weights(mt._model, rewritten_modules)
    ...
    for request in requests:
        ...
        with torch.no_grad():
            weights = nethook.get_parameter(mt._model, w_name)
            ...
            delta_k = delta_k.to(weights.dtype).to(weights.device)
            delta_v = delta_v.to(weights.dtype).to(weights.device)
            print(f"{weights.device=} | {delta_k.device=} | {delta_v.device=}")
            upd_matrix = delta_k.unsqueeze(1) @ delta_v.unsqueeze(0)
            upd_matrix = upd_matrix_match_shape(upd_matrix, weights.shape)
            weights[...] += upd_matrix.to(weights.dtype).to(weights.device)

    if return_orig_weights:
        return mt._model, weights_copy
    else:
        return mt._model
