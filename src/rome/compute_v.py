...
def compute_v(
    ...
    model, tok = mt._model, mt.tokenizer

    # Tokenize target into list of int token IDs
    target_ids = tok(
        request["target_new"]["str"], add_special_tokens=False, return_tensors="pt"
    ).to("cuda")["input_ids"][0]
    ...
    def edit_output_fn(cur_out, cur_layer):
        ...
        if cur_layer == hparams.rewrite_module_tmp.format(layer):
            ...
            for i, idx in enumerate(lookup_idxs):
                cur_out[i, idx, v_insert_range[0] : v_insert_range[1]] += delta.to(
                    cur_out.device
                )
    ...
    while it <= LIMIT:
        ...
        weight_decay = hparams.v_weight_decay * (
            torch.norm(delta.to(device)) / torch.norm(target_init.to(device)) ** 2
        )
    ...
    if left_vector is None:
        ...
        left_vector = left_vector.to(mt.device)
    ...
    # logger.debug(
    #     f"{target.device=} | {cur_input.device=} | {cur_output.device=} | {left_vector.device=}"
    # )
    cur_input = cur_input.to(mt.device)
    cur_output = cur_output.to(mt.device)
